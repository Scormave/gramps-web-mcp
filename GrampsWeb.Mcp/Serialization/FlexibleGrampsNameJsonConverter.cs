...
public sealed class FlexibleGrampsNameJsonConverter : JsonConverter<FlexibleGrampsName?>
{
        ...
        switch (reader.TokenType)
        {
            ...
            case JsonTokenType.String:
            {
                ...
                if (string.IsNullOrWhiteSpace(s))
                    return null;
                var name = ParseStringValue(s.Trim(), options);
    ...
    private static GrampsName ParseStringValue(string s, JsonSerializerOptions options) =>
        FlexibleGrampsNameParsing.ParseStringValue(s, options);
